...
describe("Ethernal Elves Contracts", function () {
  ...
  it("Mint Artifacts", async function () {
   ...
   it("Add scrolls, go on crusade", async function () {
     ...
     let balance = parseInt((await elvesPolygon.bankBalances(addr3.address))/1000000000000000000);

     await elvesPolygon.connect(addr3).buyScrolls(5,addr3.address);
     
     expect(parseInt(await elvesPolygon.scrolls(addr3.address))).to.equal(15)

   //  expect(parseInt((await elvesPolygon.bankBalances(addr3.address))/1000000000000000000)).to.equal(balance - (5*750))
     ...
     expect(parseInt(await elvesPolygon.scrolls(addr3.address))).to.equal(15-3)
